...
export const GameManager = () => {
    ...
    const isStand = useSelector((state) => state.gameManager.isStand);
    ...
    useEffect(() => {
        ...
        else if (isStand && calculateCardPoints(croupierCards) <= 21 && calculateCardPoints(croupierCards) > calculateCardPoints(playerCards)) {
            dispatch(setWhoWon('croupier'));
        }
        //Draw
        ...
        else if (isStand && calculateCardPoints(playerCards) <= 21 && calculateCardPoints(playerCards) > calculateCardPoints(croupierCards)) {
            dispatch(setWhoWon('croupier'));
        }
